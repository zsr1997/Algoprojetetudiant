...
get_indexes <- function(sums, ordered_sums, size){

  indexes = c(rep(-1,size))
  for(i in 1:S){
    item = ordered_sums[i]
...
order_matrix <- function(mat, indexes){
  S = sqrt(length(mat))
  ordered_mat = matrix(0,S,S)

  for(i in 1:S){
    idx = indexes[i]
    for(j in 1:S)
    {ordered_mat[i,j] = mat[idx,j]}
  }
  return(ordered_mat)
...
max_score <- function(M, ordered, pairs, chosen_students,indexes,stud_count, S){
  if(sqrt(length(M))==2){
    proj1 = S-1
    proj2 = S

    search_list_1 <- c(rep(0), S)
    for(i in 1:S){
      search_list_1[i] = ordered[proj1,i]
    }
    search_list_2 <- c(rep(0), S)
    for(i in 1:S){
    ...
    if(M[1,1]+ M[2,2] >= M[1,2] + M[2,1]){
      ...
      ###############stud1
      k1 = 2
      stud1 = which(search_list_1 == M[1,1] )[1]

      while((stud1 %in% chosen_students)== TRUE){
        #print("inwhile")
        stud = which(search_list_1 == M[1,1])[k1]
        k1 = k1+1
      }
      chosen_students[stud_count] = stud1[1]
      stud_count = stud_count+1
      ############################

      ########stud2
      k2 = 2
      stud2 = which(search_list_2 == M[2,2] )[1]

      while((stud2 %in% chosen_students)== TRUE){
        stud = which(search_list_2 == M[2,2])[k2]
        k2 = k2+1
      }
      chosen_students[stud_count] = stud2[1]
      stud_count = stud_count+1
      ...
      ###############stud1
      k1 = 2
      stud1 = which(search_list_1 == M[1,2] )[1]

      while((stud1 %in% chosen_students)== TRUE){
        stud = which(search_list_1 == M[1,2])[k1]
        k1 = k1+1
      }
      chosen_students[stud_count] = stud1[1]
      stud_count = stud_count+1
      ...
      ########stud2
      k2 = 2
      stud2 = which(search_list_2 == M[2,1] )[1]

      while((stud2 %in% chosen_students)== TRUE){
        stud = which(search_list_2) == M[2,1][k2]
        k2 = k2+1
      }
    ...
    pairs[S-1, 1] = stud1[1]
    pairs[S-1, 2] = indexes[proj1]

    pairs[S, 1] = stud2[1]
    pairs[S, 2] = indexes[proj2]


    print(pairs)

    #result <- list(max(M[1,1]+ M[2,2], M[1,2] + M[2,1]), pairs)


    return(max(M[1,1]+ M[2,2], M[1,2] + M[2,1]))}
    ...
    stud = -1
    print(search_list)
    stud = which(search_list == m)[1]


    while((stud %in% chosen_students)== TRUE){


      stud = which(search_list == m)[k]
      k = k+1
    }

    chosen_students[stud_count] = stud[1]
    stud_count = stud_count+1
    ...
    #updating the pairs
    pairs[stud_count - 1, 1] = stud[1]
    pairs[stud_count - 1, 2] = indexes[proj]
    #pairs[stud_count - 1, 2] = proj
    ...
    new_mat <- matrix(0, new_size, new_size)
    ...
    return(m + max_score(new_mat, ordered, pairs, chosen_students,indexes,stud_count, S ))}}
...
#S = 6
#S = 3
M<- matrix(c(1,2,3,4,5,3,4,2,1,5,4,2,5,1,3,2,3,5,1,4,5,4,1,3,2), nrow = 5, ncol = 5,byrow = TRUE)

#M<- matrix(c(1,2,3,3,1,2,2,3,1), 3, 3, byrow = TRUE)
...
ordered_sums <- sort(sums)


ordered_sums


indexes = get_indexes(sums, ordered_sums, S)

indexes

ord_mat <- order_matrix(M_T, indexes)
ord_mat

ord_mat_copy <-order_matrix(M_T, indexes)
